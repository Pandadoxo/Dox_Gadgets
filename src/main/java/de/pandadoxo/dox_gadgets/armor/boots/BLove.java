...
public class BLove extends Boots {

    public static final String NAME = "§8• §5Liebes Schuhe §8•";
    public static final GuiItem showItem = new GuiItem(NAME, Material.LEATHER_BOOTS).setLeatherArmorColor(Color.fromRGB(170, 0, 170));
    public final GuiItem realItem = new GuiItem(NAME, Material.LEATHER_BOOTS).setUnbreakable(true).setLeatherArmorColor(Color.fromRGB(170, 0, 170));

    public BLove(EBoots boots) {
        super(boots);
    ...
    void onTick() {
        for (Player all : Bukkit.getOnlinePlayers()) {
            ...
            if (gPlayer.getBoots().equals(getBoots())) {
                if (all.isSneaking()) {
                    all.getWorld().spawnParticle(Particle.HEART, all.getEyeLocation().add(0, .5, 0), 1, 0, 0, 0, 0);
                } else {
                    all.getWorld().spawnParticle(Particle.HEART, all.getLocation().add(0, 0, 0), 1, 0, 0, 0, 0);
                }
            }
        }
    }
